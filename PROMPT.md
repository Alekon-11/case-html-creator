...
<div class="formcard">
  ...
  <form class="formgrid" novalidate>
    ...
    <div class="field-wrap"><input type="tel" name="phone" inputmode="tel" placeholder="+7 (___) ___-__-__" autocomplete="tel" required><span class="form-err"></span></div>
...
### Скрипт 3 — маски и валидация формы (вставить перед `</body>`, если блок form включён)
Поля Имя, Телефон, E-mail — обязательные, с масками прямо при вводе:
- **Имя** — только буквы (рус/лат), пробел и дефис между словами; цифры/символы и лишние
  пробелы убираются на лету.
- **Телефон** — маска `+7 (999) 999-99-99`, принимаются только цифры; на выходе ровно 11 цифр.
...
(function(){
  ...
  document.querySelectorAll(".formgrid").forEach(function(form){
    ...
    function vPhone(){ if(digits(phone.value)!==11){setErr(phone,"Введите телефон полностью");return false;} setErr(phone,""); return true; }
    ...
    if(phone){
      phone.addEventListener("input",function(){ phone.value=fmtPhone(phone.value); setErr(phone,""); });
    ...
    form.addEventListener("submit",function(e){
      ...
      if(ok){ form.reset(); alert("Заявка отправлена!"); }
    });
  });
})();
</script>
